...
public class RadioTools extends Thread {
    ...
    public String SubscriptionProcessNode() {
        ...
        try {
            ...
            subsdg.writeByte(PDUs.HELLO_SINK); // packet type
            ...
            while (true) {
                ...
                if (subsdgrx.readByte() == PDUs.HELLOSINK_RESPONSE){
    ...
    public void doSetMeasures (float tempValue, float ecValue, float phValue, String SinkAddress){
        
        short ACK=0;

        DatagramConnection conntx = null;      
        try {
            conntx = (DatagramConnection) Connector.open("radiogram://" + SinkAddress + ":" + PDUs.CONNECTED_PORT); //
    ...
    public void doSendMeasures (float tempValue, float ecValue, float phValue, String SinkAddress){
        DatagramConnection conntx = null;
        try {
            conntx = (DatagramConnection) Connector.open("radiogram://" + SinkAddress + ":" + PDUs.CONNECTED_PORT); //
            Datagram dg = conntx.newDatagram(conntx.getMaximumLength());
    ...
    private void announceStarting () {
        ...
        try{
            txConn = (DatagramConnection)Connector.open("radiogram://broadcast:" + PDUs.CONNECTEDSINK_PORT);
